...
xsetcolorname(int x, const char *name)
{
	Color ncolor;
        ...
        /* support setting the alpha value like urxvt */
        char *lbracket_loc = strchr(name, '[');
        if (lbracket_loc) {
            char *rbracket_loc = NULL;
            float newalpha = (float) (strtol(lbracket_loc + 1, &rbracket_loc, 0) % 100 / 100.0);
            alpha = newalpha > 0.0 ? newalpha : 1.0;
            name = rbracket_loc + 1;
        } else if(resetalpha) {
            alpha = 1.0;
        }
	...
	if (!xloadcolor(x, name, &ncolor))
        ...
        if (x == defaultbg) {
            ncolor.color.alpha = (unsigned short)(0xffff * alpha);
            ncolor.pixel &= 0x00FFFFFF;
            ncolor.pixel |= (unsigned char)(0xff * alpha) << 24;
        }
